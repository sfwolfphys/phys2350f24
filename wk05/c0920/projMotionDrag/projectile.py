...
def velComps(vLaunch,launchAngleDeg):
    ...
    velocity = [vLaunch * np.cos(launchAngleRad), 
                    vLaunch * np.sin(launchAngleRad)]
    return velocity
...
def genProjectilePlot(xi,yi,vLaunch,thetaLaunch, dt, m, c):
    ...
    maxDist = np.max(mA.xpos)
    theTitle = f'The range of the cannonball is {maxDist:.1f} m'

    # Make the plot
    fig, ax = plt.subplots(nrows=2,ncols=2, sharex=True)
    fig.suptitle(theTitle)
